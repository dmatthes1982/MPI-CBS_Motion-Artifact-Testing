...
if ~exist('sessionStr', 'var')
  ...
  cfg.filename  = 'MAT_d01_01a_raw';
  sessionNum    = MAT_getSessionNum( cfg );
  if sessionNum == 0
    sessionNum = 1;
  end
...
if ~exist('srcPath', 'var')
  srcPath = ['/data/pt_01826/eegData_MotionArtifactTesting/'                % source path for raw data 
              'DualEEG_MAT_rawData/']; 
end
...
if ~exist('numOfPart', 'var')                                               % estimate number of participants in raw data folder
  ...
  for i=1:1:numOfSources
    numOfPart(i)  = sscanf(sourceList{i}, 'DualEEG_MAT_%d.vhdr');
  end
end
...
for i = numOfPart
  ...
  ft_info off;
  data_raw = MAT_importDataset( cfg );
  ft_info on;
  ...
  fprintf('%s ...\n', file_path);
  MAT_saveData(cfg, 'data_raw', data_raw);
  fprintf('Data stored!\n\n');
  clear data_raw
end
...
if ~(exist(settings_file, 'file') == 2)                                     % check if settings file already exist
  ...
  MAT_createTbl(cfg);                                                       % create settings file
end
...
for i = numOfPart
  ...
  data_continuous = MAT_concatData( data_raw );
  
  fprintf('\n');
  
  % select corrupted channels
  data_badchan = MAT_selectBadChan( data_continuous );
  
  % export the bad channels in a *.mat file
  cfg             = [];
  cfg.desFolder   = strcat(desPath, '01b_badchan/');
  cfg.filename    = sprintf('MAT_d%02d_01b_badchan', i);
  cfg.sessionStr  = sessionStr;
  ...
  fprintf('%s ...\n', file_path);
  MAT_saveData(cfg, 'data_badchan', data_badchan);
  fprintf('Data stored!\n\n');
  ...
  data_repaired = MAT_repairBadChan( data_badchan, data_raw );
  
  % export the bad channels in a *.mat file
  cfg             = [];
  cfg.desFolder   = strcat(desPath, '01c_repaired/');
  cfg.filename    = sprintf('MAT_d%02d_01c_repaired', i);
  cfg.sessionStr  = sessionStr;
  ...
  fprintf('%s ...\n', file_path);
  MAT_saveData(cfg, 'data_repaired', data_repaired);
  fprintf('Data stored!\n\n');
